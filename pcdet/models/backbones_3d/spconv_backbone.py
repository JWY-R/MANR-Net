...
from .rectification_prompter import RectificationPrompterLayer
from .shape_aware_unit import ShapeAwareUnit3D
...
class MANRNet(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size,  **kwargs):
        ...
        self.shape_aware_unit = ShapeAwareUnit3D(
            input_channels=self.out_features,
            output_channels=self.out_features,
            hidden_channels=64
        )
        if self.model_cfg.get('MM', False):
            self.vir_conv1 = RectificationPrompterLayer(input_channels, num_filters[0], stride=1, indice_key='vir1')
            self.vir_conv2 = RectificationPrompterLayer(num_filters[0], num_filters[1], stride=2, indice_key='vir2')
            self.vir_conv3 = RectificationPrompterLayer(num_filters[1], num_filters[2], stride=2, indice_key='vir3')
            self.vir_conv4 = RectificationPrompterLayer(num_filters[2], num_filters[3], stride=2, padding=(0, 1, 1),
                                          indice_key='vir4')
